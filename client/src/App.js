...
function AlbumCard ({ id, name, images, release_date, external_urls: {spotify: url }}) {
      ...
      {(images && images.length > 0)
        ...
        : (
          <div
            ...
            style={{ width: '160px', height: '160px', border: 'solid 1px #1db954' }}
           >
            <a href={url}>{name}</a>
            <div className="year">{year}</div>
          </div>
  ...
  render() {
    ...
    return (
      <>
        ...
        <h1 className="page-details-header">
          <a href={`${BASE_WEB_PLAYER_URL}/artist/${id}`}>
            {name}
          </a>
        </h1>
